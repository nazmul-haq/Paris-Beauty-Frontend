   ...
   useEffect(() => {
     if (router?.query?.slug) {
       const getData = async () => {
         let res = await request(
           `get-campaign-details/${router.query.slug}?page=${page}`
         );
    ...
    const loadMoreUsers = async () => {
      ...
      const res = await request(
        `get-campaign-details/${router.query.slug}?page=${page + 1}`
      );
...
// try {
//   let res = await request(`get-campaign-details/${context.query.slug}?page=1`);
...
//      return {
//        props: {
//          Data: res?.products?.data || null,
//          PageSize: res?.products?.per_page || null,
//          TotalData: res?.products?.total || null,
//          category_name: res?.campign_details?.name || null,
//        },
//      };
...
// } catch (error) {
//   console.error("Error fetching campaign-details product data:", error);
//   return {
//     notFound: true,
//   };
// }
